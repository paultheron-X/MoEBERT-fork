...
class LearnPermutationsT(torch.nn.Module):
    """Learn permutations.
    Input:
    ...
    def __init__(
        ...
        super(LearnPermutationsT, self).__init__()
        ...
        self.tau_ref = torch.tensor(
            np.linspace(
                np.log10(self.tau_initial), np.log10(self.tau_final), num = 2
            ),
            dtype=torch.float32,
        )
        ...
        self.permutation_weights = torch.nn.Parameter(
            torch.tensor(
                np.array([np.random.uniform(size = (self.nb_experts, self.nb_experts)) for _ in range(self.no_of_permutations)]), dtype=torch.float32
            )
        )
        ...
        self.permutation_log_weights = torch.nn.Parameter(
            torch.tensor(
                np.array([np.zeros(shape = (self.nb_experts, self.nb_experts)) for _ in range(self.no_of_permutations)]), dtype=torch.float32
            )
        )
    ...
    def _sinkhorn(self, log_weights, tau, eps=1e-20):
        weights = torch.exp(log_weights / tau)
        weights /= torch.sum(weights, dim=2, keepdim=True) + eps
        weights /= torch.sum(weights, dim=1, keepdim=True) + eps
        return weights
    
    def _gumbel_sinkhorn(self, log_weights, tau, eps=1e-20):
        weights = self._sinkhorn(log_weights - self._sample_gumbel(log_weights.shape), tau, eps)
        return weights

    def _generate_mask_per_permutation(self, weights, eps=1e-20):
        mask = torch.zeros(weights.shape)
        for i in range(weights.shape[0]):
            mask[i] = torch.eye(weights.shape[1])
        mask = mask.to(weights.device)
        return mask
    
    def _get_permutation_mask(self, weights, eps=1e-20):
        mask = torch.zeros(weights.shape)
        for i in range(weights.shape[0]):
            mask[i] = torch.eye(weights.shape[1])
        mask = mask.to(weights.device)
    ...
    def _compute_permutation_entropy_regularization(self, weights, eps=1e-20):
        weights = weights + eps
        weights = weights / torch.sum(weights, dim=2, keepdim=True)
        weights = weights / torch.sum(weights, dim=1, keepdim=True)
        entropy = -torch.sum(weights * torch.log(weights + eps), dim=(1, 2))
        return entropy
    ...
    def _get_permutation_during_training(
        ...
        permutation_weights = self._gumbel_sinkhorn(
            permutation_weights, tau, eps=eps
        )
    ...
    def _get_permutation_during_inference(
        ...
        permutation_weights = self._sinkhorn(
            permutation_weights, tau, eps=eps
        )
        permutation_weights = self._get_permutation_mask(permutation_weights)
        #         tf.print("====iteration:", self.iterations, "==perm", permutation_weights)
    ...
    def forward(self, x, tau, iterations, eps=1e-20):
        if iterations < self.iterations_for_learning_permutation:
            permutation_weights = self._get_permutation_during_training(
                self.permutation_weights, tau, eps=eps
            )
        else:
            permutation_weights = self._get_permutation_during_inference(
                self.permutation_weights, tau, eps=eps
            )
        return permutation_weights
...
class LearnPermutations(tf.keras.layers.Layer):
    ...
    def _tf_linear_sum_assignment(self, cost_matrix):
        return tf.numpy_function(
            func=scipy.optimize.linear_sum_assignment, inp=[cost_matrix], Tout=[tf.int64, tf.int64]
        )
    ...
    def _generate_mask_per_permutation(self, permutation_weights):
        ...
        permutation_mask = tf.gather(tf.eye(tf.shape(permutation_weights)[-1]), col_ind, axis=0)
    ...
    def _compute_permutation_entropy_regularization(
        ...
        regularization = tf.reduce_mean(
            tf.math.reduce_sum(-permutation_weights * tf.math.log(permutation_weights + eps), axis=[1, 2])
        ) + tf.reduce_mean(
            tf.math.reduce_sum(
                -permutation_weights_row_norm * tf.math.log(permutation_weights_row_norm + eps), axis=[1, 2]
            )
        )
    ...
    def _get_permutation_during_training(
        ...
        tau = tf.math.pow(tf.constant(10.0), log_tau)

        permutation_log_weights = tf.cond(
            tf.math.greater_equal(
                self.iterations, tf.cast(self.iterations_for_learning_permutation, dtype=self.iterations.dtype)
            ),
        ...
        permutation_weights = self._gumbel_sinkhorn(
            permutation_log_weights, temp=tau, n_samples=1, noise_factor=noise_factor, n_iters=n_iters
        )
    ...
    def _get_permutation_during_learning_and_after_learning(self, iterations):
        norm = tf.linalg.norm(
            self._get_permutation_during_inference(self.permutation_weights) - self.permutation_weights, axis=[1, 2]
        )
        #         tf.print("=====norm:", norm)
        ...
        permutation_weights = tf.cond(
            tf.math.less(iterations, tf.cast(self.iterations_for_learning_permutation, dtype=self.iterations.dtype)),
            lambda: self._get_permutation_during_training(
                self.permutation_log_weights, noise_factor=self.noise_factor
            ),
    ...
    def call(self, inputs, training=True):
        ...
        increment = control_flow_util.smart_cond(
            training, lambda: tf.ones_like(self.iterations), lambda: tf.zeros_like(self.iterations)
        )
        ...
        self.add_loss(self.perm_entropy_reg * explicit_perm_entropy_regularization)
